...
def thresholding(img):
    img_hsv = cv2.cvtColor(img,cv2.COLOR_BGR2HSV)  #convert into HSV
    lowerWhite = np.array([80,0,0])
    upperWhite = np.array([179,160,255])
    mask= cv2.inRange(img_hsv,lowerWhite,upperWhite)
...
def drawPoints(img,points):
    for p in range(4):
        cv2.circle(img,(int(points[p][0]),int(points[p][1])),15,(0,0,255),cv2.FILLED)
...
def getHistogram(img,minPer= 0.1, display = False):
    ...
    index_arr = np.where(histValues >= minVal)
    ...
    if display:
        ...
        for x, intensity in enumerate(histValues):
            ...
            cv2.circle(imgHist,(base_point,img.shape[0]),20,(0,255,255),cv2.FILLED)
        return base_point,imgHist    
    return base_point
